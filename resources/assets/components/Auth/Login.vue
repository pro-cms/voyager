        ...
        <form method="post" :action="route('voyager.login')" v-if="!passwordForgotOpen" key="login-form">
            ...
            <slot name="login">
                <div class="w-full mt-1">
                    <label for="email" class="block text-sm font-medium leading-5">{{ __('voyager::auth.email') }}</label>
                    <div class="mt-1 rounded-md shadow-sm">
                        <input type="email" name="email" id="email" class="voyager-input w-full mb-4 placeholder-gray-400" autofocus>
                    </div>
                </div>
                <div class="w-full mt-6">
                    <label for="password" class="block text-sm font-medium leading-5">{{ __('voyager::auth.password') }}</label>
                    <div class="mt-1 rounded-md shadow-sm">
            ...
            <div class="flex items-center justify-between mt-4">
                <button dusk="login-button" class="button accent large w-full justify-center" type="submit">
